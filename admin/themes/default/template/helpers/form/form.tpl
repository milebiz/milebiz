				...
				{elseif $key == 'input'}
					{foreach $field as $input}
						...
						{else}
							...
							{/if}
							{if $input.name == 'id_city'}
								<div id="contains_cities" {if !$contains_states}style="display:none;"{/if}>
							{/if}
							{block name="label"}
							...
							{if $input.name == 'id_state' || $input.name == 'id_city'}
								</div>
							{/if}
						{/if}
					{/foreach}
...
{if $firstCall}
	<script type="text/javascript">
		...
		$(document).ready(function() {
		    id_current_state = 0;
		    id_current_city = 0;
			{if isset($fields_value.id_state) && $fields_value.id_state != ''}
			    id_current_state = {$fields_value.id_state};
			{/if}
			{if isset($fields_value.id_city) && $fields_value.id_city != ''}
			    id_current_city = {$fields_value.id_city};
			{/if}
			if ($('#id_country') && $('#id_state'))
			{
				ajaxStates(id_current_state);
				ajaxCity(id_current_city,id_current_state);
