						...
						<div style="margin-bottom:5px;">
							<p>
								<select name="id_address">
									...
									<option value="{$address['id_address']}"{if $address['id_address'] == $order->id_address_delivery} selected="selected"{/if}>{$address['alias']} - {$address['country']} {if !empty($address['state'])} {$address['state']}{/if} {$address['city']} {$address['postcode']} {$address['address1']}</option>
									{/foreach}
								</select>
				...
				<form method="post" action="{$link->getAdminLink('AdminOrders')|escape:'htmlall':'UTF-8'}&vieworder&id_order={$order->id}">
					<div style="margin-bottom:5px;">
						<p>
							<select name="id_address">
								{foreach from=$customer_addresses item=address}
								<option value="{$address['id_address']}"{if $address['id_address'] == $order->id_address_invoice} selected="selected"{/if}>{$address['alias']} - {$address['country']} {if !empty($address['state'])} {$address['state']}{/if} {$address['city']} {$address['postcode']} {$address['address1']}</option>
								{/foreach}
							</select>
