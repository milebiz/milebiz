...
{block name="override_tpl"}
	...
	<div id="container-customer">
		<h2><img src="../img/admin/basket.png" /><span>{if $customer->id}{$customer->lastname}{$customer->firstname}{else}{l s='Guest'}{/if} - {l s='Cart #'}{$cart->id|string_format:"%06d"}</span><span style="color:#585A69;padding-left:10px;">{l s='on'} {$cart->date_upd}</span></h2>
	...
	<div class="info-customer-left">
		...
		{if $customer->id}
			<a href="{$link->getAdminLink('AdminCustomers')|escape:'htmlall':'UTF-8'}&id_customer={$customer->id}&viewcustomer">{$customer->lastname}{$customer->firstname}</a></span>
			{l s='#'}{$customer->id}<br />
