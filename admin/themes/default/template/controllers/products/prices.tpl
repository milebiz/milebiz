		...
		"success": function(result)
		{
			if(result.found) {
				...
				jQuery.each(result.customers, function() {
					html += '<li><a class="fancybox" href="{$link->getAdminLink('AdminCustomers')}&id_customer='+this.id_customer+'&viewcustomer&liteDisplaying=1">'+this.lastname+this.firstname+'</a>'+(this.birthday ? ' - '+this.birthday:'')+'<br/>';
					html += '<a href="mailto:'+this.email+'">'+this.email+'</a><br />';
					html += '<a onclick="Customer.select('+this.id_customer+', \''+this.lastname+this.firstname+'\'); return false;" href="#" class="button">{l s='Choose'}</a></li>';
				});
				html += '</ul>';
			}
			else
		...
		<script type="text/javascript">
			$(document).ready(function(){
				...
				$('.datepicker').datetimepicker({
					...
					// Define a custom regional settings in order to use MileBiz translation tools
					currentText: '{l s='Now'}',
					closeText: '{l s='Done'}',
					ampm: false,
					amNames: ['AM', 'A'],
