...
if (Tools::isSubmit('installBoughtModule'))
{
	...
	$sourceFile = 'http://addons.milebiz.com/iframe/getboughtfile.php?id_order_detail='.Tools::getValue('id_order_detail').'&token='.Tools::getValue('token');
	if (!copy($sourceFile, $file))
	{
		if (!($content = file_get_contents($sourceFile)))
			die(displayJavascriptAlert('Access denied: Please download your module directly from MileBiz Addons website'));
...
if (Tools::getValue('page') == 'prestastore' AND @fsockopen('addons.milebiz.com', 80, $errno, $errst, 3))
	readfile('http://addons.milebiz.com/adminmodules.php?lang='.$context->language->iso_code);
