...
abstract class ModuleCore
{
	...
	/**
	 * @since 1.5.0.1
	 * @var string Module local path (eg. '/home/milebiz/modules/modulename/')
	 */
	...
	public function install()
	{
		...
		{
			$this->_errors[] = $this->l('The version of your module is not compliant with your MileBiz version.');
			return false;
		}
		...
		if (!$result)
		{
			$this->_errors[] = $this->l('Technical error : MileBiz could not installed this module.');
			return false;
		}
	...
	/**
	  ...
	  * @param boolean $use_vars_instead_of_ids use an js vars instead of ids seperate by "陇"
	  */
	...
	public function registerHook($hook_name, $shop_list = null)
	{
		...
		if (!Validate::isHookName($hook_name))
			throw new MileBizException('Invalid hook name');
		...
		foreach ($modules as $name)
		{
			if (is_dir(_PS_MODULE_DIR_.$name) && Tools::file_exists_cache(_PS_MODULE_DIR_.$name.'/'.$name.'.php'))
			{
				if (!Validate::isModuleName($name))
					throw new MileBizException(sprintf('Module %s is not a valid module name', $name));
				$module_list[] = $name;
			}
		}
		return $module_list;
	...
	public function commonL($string, $specific = false, $id_lang = null)
	{
		if (self::$_generate_config_xml_mode)
			return $string;
	    global $_COMMON, $cookie;
		...
		@include_once(_PS_TRANSLATIONS_DIR_.$iso.'/common.php');

		if (!is_array($_COMMON))
			return str_replace('"', '&quot;', $string);
		$key = md5(str_replace('\'', '\\\'', $string));
		$str = (isset($_COMMON) AND is_array($_COMMON) AND key_exists($key, $_COMMON)) ? $_COMMON[$key] : $string;
	...
	*/
	...
	public static function getPermissionStatic($id_module, $variable, $employee = null)
	{
		...
		if (!isset(self::$cache_permissions[$employee->id_profile][$id_module]))
			throw new MileBizException('No access reference in table module_access for id_module '.$id_module.'.');
	...
	public function removeOverride($classname)
	{
		...
		foreach ($module_class->getProperties() as $property)
		{
			...
			// Remplacer la ligne de d茅claration par "remove"
