...
abstract class DbCore
{
	...
	public function autoExecute($table, $data, $type, $where = '', $limit = 0, $use_cache = true, $use_null = false)
	{
		...
		switch ($type)
		{
			...
			default :
				throw new MileBizDatabaseException('Wrong argument (miss type) in Db::autoExecute()');
		}
	}
	...
	public function insert($table, $data, $null_values = false, $use_cache = true, $type = Db::INSERT, $add_prefix = true)
	{
		...
		if ($type == Db::INSERT)
			...
			throw new MileBizDatabaseException('Bad keyword, must be Db::INSERT or Db::INSERT_IGNORE or Db::REPLACE');
			...
			foreach ($row_data as $key => $value)
			{
				if (isset($keys_stringified))
				{
					...
					if (!in_array("`$key`", $keys))
						throw new MileBizDatabaseException('Keys form $data subarray don\'t match');
				}
				else
					$keys[] = "`$key`";
	...
	public function executeS($sql, $array = true, $use_cache = true)
	{
		...
		if (!preg_match('#^\s*\(?\s*(select|show|explain|describe|desc)\s#i', $sql))
		{
			if (defined('_PS_MODE_DEV_') && _PS_MODE_DEV_)
				throw new MileBizDatabaseException('Db->executeS() must be used only with select, show, explain or describe queries');
	...
	public function displayError($sql = false)
	{
		...
		{
			if ($sql)
				throw new MileBizDatabaseException($this->getMsgError().'<br /><br /><pre>'.$sql.'</pre>');
			throw new MileBizDatabaseException($this->getMsgError());
		}
	}
