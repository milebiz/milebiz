...
class MySQLCore extends Db
{
	...
	public function connect()
	{
		...
		if (!$this->link = mysql_connect($this->server, $this->user, $this->password))
			throw new MileBizDatabaseException(Tools::displayError('Link to database cannot be established.'));

		if (!$this->set_db($this->database))
			throw new MileBizDatabaseException(Tools::displayError('The database selection cannot be made.'));
