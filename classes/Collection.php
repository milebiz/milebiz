...
class CollectionCore implements Iterator, ArrayAccess, Countable
{
	...
	public function __construct($classname, $id_lang = null)
	{
		...
		if (!isset($this->definition['table']))
			throw new MileBizException('Miss table in definition for class '.$this->classname);
		else if (!isset($this->definition['primary']))
			throw new MileBizException('Miss primary in definition for class '.$this->classname);
	...
	public function where($field, $operator, $value, $method = 'where')
	{
		if ($method != 'where' && $method != 'having')
			throw new MileBizException('Bad method argument for where() method (should be "where" or "having")');
		...
		if (is_array($value))
		{
			switch (strtolower($operator))
			{
				...
				default :
					throw new MileBizException('Operator not supported for array value');
			}
		}
		// Create WHERE clause
		else
		{
			switch (strtolower($operator))
			{
				...
				default :
					throw new MileBizException('Operator not supported');
			}
		}
	...
	public function orderBy($field, $order = 'asc')
	{
		...
		if ($order != 'asc' && $order != 'desc')
			throw new MileBizException('Order must be asc or desc');
	...
	public function offsetGet($offset)
	{
		...
		if (!isset($this->results[$offset]))
			throw new MileBizException('Unknown offset '.$offset.' for collection '.$this->classname);
	...
	public function offsetSet($offset, $value)
	{
		if (!$value instanceof $this->classname)
			throw new MileBizException('You cannot add an element which is not an instance of '.$this->classname);
		...
		if (!isset($this->association_definition[$association]))
		{
			...
			for ($i = 0, $total_association = count($split); $i < $total_association; $i++)
			{
				...
				if (!isset($definition['associations'][$asso]))
					throw new MileBizException('Association '.$asso.' not found for class '.$this->definition['classname']);
		...
		if (!isset($this->fields[$field]))
		{
			...
			else
			{
				...
				if (!isset($definition['fields'][$fieldname]))
					throw new MileBizException('Field '.$fieldname.' not found in class '.$definition['classname']);

				// Test field validity for language fields
				if (empty($definition['is_lang']) && !empty($definition['fields'][$fieldname]['lang']))
					throw new MileBizException('Field '.$fieldname.' is declared as lang field but is used in non multilang context');
				else if (!empty($definition['is_lang']) && empty($definition['fields'][$fieldname]['lang']))
					throw new MileBizException('Field '.$fieldname.' is not declared as lang field but is used in multilang context');
