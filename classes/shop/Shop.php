	...
	public static function initialize()
	{
		...
		else
		{
			...
			if (!Validate::isLoadedObject($shop) || !$shop->active || !$id_shop)
			{
				...
				// Hmm there is something really bad in your MileBiz !
				if (!Validate::isLoadedObject($default_shop))
					throw new MileBizException('Shop not found');
	...
	*/
	public function getAddress()
	{
		if (!isset($this->address))
		{
			...
			$address->id_city = Configuration::get('PS_SHOP_CITY_ID');
	...
	public static function setContext($type, $id = null)
	{
		switch ($type)
		{
			...
			default :
				throw new MileBizException('Unknown context for shop');
		}
