	...
	*/
		...
		$secure_key = false, Shop $shop = null)
	{
		...
		if (!Validate::isLoadedObject($order_status))
			throw new MileBizException('Can\'t load Order state status');
		...
		if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false)
		{
			...
			foreach ($package_list as $id_address => $packageByAddress)
				foreach ($packageByAddress as $id_package => $package)
				{
					...
					if (!$result)
						throw new MileBizException('Can\'t save Order');
			...
			if ($order_status->logable)
			{
				...
				if (!$order->addOrderPayment($amount_paid, null, $transaction_id))
					throw new MileBizException('Can\'t save Order Payment');
			}
			...
			foreach ($order_detail_list as $key => $order_detail)
			{
				...
				if (!$order_creation_failed & isset($order->id))
				{
					...
					if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id)
					{
						...
						$delivery_city = $delivery->id_city ? new City($delivery->id_city) : false;
						$invoice_city = $invoice->id_city ? new City($invoice->id_city) : false;
						...
						'{delivery_city}' => $delivery->id_city ? $delivery_city->name : '',
						...
						'{invoice_city}' => $invoice->id_city ? $invoice_city->name : '',
	...
	/**
	 * @param int $id_currency : this parameter is optionnal but on 1.5 version of MileBiz, it will be REQUIRED
	 * @return Currency
	 */
