...
class LocalizationPackCore
{
	...
	protected function _installCurrencies($xml, $install_mode = false)
	{
		if (isset($xml->currencies->currency))
		{
            ...
            if (!$feed = Tools::simplexml_load_file('http://api.milebiz.com/xml/currencies.xml'))
		...
		if (isset($xml->languages->language))
			foreach ($xml->languages->language as $data)
			{
				...
				if (!$install_mode || !in_array((string)$attributes['iso_code'], $native_iso_code))
				{
					$errno = 0;
					$errstr = '';
					if (!@fsockopen('api.milebiz.com', 80, $errno, $errstr, 5))
						$this->_errors[] = Tools::displayError('Archive cannot be downloaded from milebiz.com.');
					elseif (!($lang_pack = Tools::jsonDecode(Tools::file_get_contents('http://www.milebiz.com/download/lang_packs/get_language_pack.php?version='._PS_VERSION_.'&iso_lang='.$attributes['iso_code']))))
						$this->_errors[] = Tools::displayError('Error occurred when language was checked according to your MileBiz version.');
					elseif ($content = Tools::file_get_contents('http://translations.milebiz.com/download/lang_packs/gzip/'.$lang_pack->version.'/'.$attributes['iso_code'].'.gzip'))
					{
