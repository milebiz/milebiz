...
class ToolsCore
{
	...
	*/
	public static function displayDate($date, $id_lang, $full = false, $separator = '-')
	{
		...
		if (!Validate::isDate($date) || !Validate::isBool($full))
			throw new MileBizException('Invalid date');
	...
	*/
	...
	public static function generateHtaccess($path = null, $rewrite_settings = null, $cache_control = null, $specific = '', $disable_multiviews = null, $medias = false)
	{
		...
		// Check current content of .htaccess and save all code outside of milebiz comments
		...
		if (file_exists($path))
		{
			...
			else
			{
				...
				if (preg_match('#\# http://www\.milebiz\.com - http://www\.milebiz\.com/forums\s*(.*)<IfModule mod_rewrite\.c>#si', $content, $m))
					$specific_before = $m[1];
				else
		...
		fwrite($write_fd, "# ~~start~~ Do not remove this comment, MileBiz will keep automatically the code outside this comment when .htaccess will be generated again\n");
		fwrite($write_fd, "# .htaccess automaticaly generated by MileBiz e-commerce open-source solution\n");
...
<IfModule mod_deflate.c>
	...
	AddOutputFilterByType DEFLATE application/x-javascript
		...
		fwrite($write_fd, "# ~~end~~ Do not remove this comment, MileBiz will keep automatically the code outside this comment when .htaccess will be generated again\n");
		fwrite($write_fd, "\n\n".trim($specific_after));
		fclose($write_fd);
