...
abstract class ObjectModelCore
{
	...
	public function __construct($id = null, $id_lang = null, $id_shop = null)
	{
		...
		if ($this->isMultishop() && !$this->id_shop)
	 	...
	 	if (!Validate::isTableOrIdentifier($this->def['primary']) || !Validate::isTableOrIdentifier($this->def['table']))
			throw new MileBizException('Identifier or table format not valid for class '.get_class($this));
				...
				foreach ($fields as $field)
				{
					foreach (array_keys($field) as $key)
						if (!Validate::isTableOrIdentifier($key))
							throw new MileBizException('key '.$key.' is not table or identifier, ');
		...
		if (isset($this->def['multilang']) && $this->def['multilang'])
		{
			...
			if (is_array($fields))
			{
				foreach ($fields as $field)
				{
					foreach (array_keys($field) as $key)
						if (!Validate::isTableOrIdentifier($key))
							throw new MileBizException('key '.$key.' is not a valid table or identifier');
	...
	public function toggleStatus()
	{
	 	...
	 	if (!array_key_exists('active', $this))
			throw new MileBizException('property "active" is missing in object '.get_class($this));
	...
	protected function makeTranslationFields(&$fields, &$fields_array, $id_language)
	{
		...
		foreach ($fields_array as $k => $field)
		{
			...
			if (!Validate::isTableOrIdentifier($field_name))
				throw new MileBizException('identifier is not table or identifier : '.$field_name);
	...
	public function validateFields($die = true, $error_return = false)
	{
		foreach ($this->def['fields'] as $field => $data)
		{
			...
			{
				if ($die)
					throw new MileBizException($message);
	...
	public function validateField($field, $value, $id_lang = null)
	{
		...
		if (!empty($data['validate']))
		{
			if (!method_exists('Validate', $data['validate']))
				throw new MileBizException('Validation function not found. '.$data['validate']);
	...
	*/
	...
	public static function hydrateCollection($class, array $datas, $id_lang = null)
	{
		if (!class_exists($class))
			throw new MileBizException("Class '$class' not found");
		...
		if ($datas)
		{
			...
			if (!array_key_exists($definition['primary'], $datas[0]))
				throw new MileBizException("Identifier '{$definition['primary']}' not found for class '$class'");
	...
	public function getFieldByLang($field_name, $id_lang = null)
	{
		...
		if ($definition && isset($definition['fields'][$field_name]))
		{
			...
			throw new MileBizException('Could not load field from definition.');
	}
