...
class InstallModelInstall extends InstallAbstractModel
{
	...
	public function generateSettingsFile($database_server, $database_login, $database_password, $database_name, $database_prefix, $database_engine)
	{
		...
		$settings_constants = array(
			...
			'_MB_VERSION_' => 			_MB_INSTALL_VERSION_,
		);
	...
	public function installLanguages()
	{
		...
		foreach ($this->language->getIsoList() as $iso)
		{
			if (!file_exists(_PS_INSTALL_LANGS_PATH_.$iso.'/language.xml'))
				throw new MileBizInstallerException($this->language->l('File "language.xml" not found for language iso "%s"', $iso));

			if (!$xml = @simplexml_load_file(_PS_INSTALL_LANGS_PATH_.$iso.'/language.xml'))
				throw new MileBizInstallerException($this->language->l('File "language.xml" not valid for language iso "%s"', $iso));
			...
			if (!$language->add())
				throw new MileBizInstallerException($this->language->l('Cannot install language "%s"', ($xml->name) ? $xml->name : $iso));
			...
			if (is_writable(_PS_IMG_DIR_.'l/'))
				if (!copy(_PS_INSTALL_LANGS_PATH_.$iso.'/flag.jpg', _PS_IMG_DIR_.'l/'.$language->id.'.jpg'))
					throw new MileBizInstallerException($this->language->l('Cannot copy flag language "%s"', _PS_INSTALL_LANGS_PATH_.$iso.'/flag.jpg => '._PS_IMG_DIR_.'l/'.$language->id.'.jpg'));
		}
	...
	public function configureShop(array $data = array())
	{
		...
		Configuration::updateGlobalValue('MB_INSTALL_VERSION', 			_MB_INSTALL_VERSION_);
		...
		$version = str_replace('.', '', _MB_VERSION_);
		$version = substr($version, 0, 2);

		$localization_file_content = @Tools::file_get_contents('http://api.milebiz.com/localization/'.$version.'/'.$data['shop_country'].'.xml');
