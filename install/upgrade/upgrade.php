...
$oldversion = _MB_VERSION_;

$versionCompare =  version_compare(_MB_INSTALL_VERSION_, $oldversion);
...
{
	$logger->logError(sprintf('You already have the %s version.',_MB_INSTALL_VERSION_));
...
{
	if (version_compare($version, $oldversion) == 1 AND version_compare(_MB_INSTALL_VERSION_, $version) != -1)
...
$datas = array(
	...
	array('_MB_VERSION_', _MB_INSTALL_VERSION_)
);
...
if (empty($fail_result))
{
	...
	Configuration::updateValue('MB_VERSION_DB', _MB_INSTALL_VERSION_);
