...
class InstallXmlLoader
{
	...
	public function populateEntity($entity)
	{
		...
		// Read list of fields
		if (!$xml->fields)
			throw new MileBizInstallerException('List of fields not found for entity '.$entity);
	...
	protected function loadEntity($entity, $iso = null)
	{
		if (!isset($this->cache_xml_entity[$this->path_type][$entity][$iso]))
		{
			...
			if (!file_exists($path))
				throw new MileBizInstallerException('XML data file '.$entity.'.xml not found');
			...
			if (!$this->cache_xml_entity[$this->path_type][$entity][$iso])
				throw new MileBizInstallerException('XML data file '.$entity.'.xml invalid');
		}
