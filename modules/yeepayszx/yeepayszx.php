...
class YeepaySzx extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
	    ...
	    $this->displayName = $this->l('yeepayszx');
	    $this->description = $this->l('Accepts payments with yeepayszx.');
	...
	public function createSuccessedOrder($cart_id)
	{
	    ...
	    if(!$cart->OrderExists()){
			...
			$yeepayszx->validateOrder($cart->id, _PS_OS_PAYMENT_, $total, $yeepayszx->displayName);
		}
		...
		'<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
			<fieldset>
				...
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td colspan="2">'.$this->l('Please enter the yeepayszx configuration information').'.<br /><br /></td></tr>';
		foreach($this->_fieldsList as $field => $detail){
			$this->_html .= '
			<tr>
