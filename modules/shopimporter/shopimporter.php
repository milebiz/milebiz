...
class shopimporter extends ImportModule
{
	...
	public function __construct()
	{
		...
		$this->description = $this->l('This module allows you to import your shop from another system into MileBiz.');
...
*/
	...
	public function genericImport($className, $fields, $save = false)
	{
			...
			if (Validate::isModuleName($moduleName) && Validate::file_exists('../../modules/'.$moduleName.'/'.$moduleName.'.php'))
			{
				...
				if (Tools::isSubmit('syncLang') || Tools::isSubmit('syncLangWS'))
				{
					...
					if ($defaultLanguage->iso_code != $defaultLanguageImport->iso_code)
						$errors[] = $this->l('Default language doesn\'t match : ').'<br>'.Configuration::get('PS_SHOP_NAME').' : '.$defaultLanguage->name.' â‰ 
				...
				if (Tools::isSubmit('syncCurrency') || Tools::isSubmit('syncCurrencyWS'))
				{
					...
					if ($defaultCurrency->iso_code != $defaultCurrencyImport->iso_code)
						$errors[] = $this->l('Default currency doesn\'t match : ').'<br>'.Configuration::get('PS_SHOP_NAME').' : '.$defaultCurrency->name.' â‰  '.$importModule->displayName.' : '.$defaultCurrencyImport->name.'<br>'.$this->l('Please change default currency in your configuration');
				}
				if (!empty($errors))
		...
		{
			//add language if not exist in milebiz
	...
	public function checkAndAddLang ($languages, $add = true)
	{
		...
		foreach($languages as $language)
		{
			...
			if (!Language::isInstalled($iso))
			{
				if ($add)
				{
					if (@fsockopen('www.milebiz.com', 80))
					{
						if ($lang_pack = Tools::jsonDecode(Tools::file_get_contents('http://www.milebiz.com/download/lang_packs/get_language_pack.php?version='._PS_VERSION_.'&iso_lang='.$iso)))
						{
							if ($content = Tools::file_get_contents('http://www.milebiz.com/download/lang_packs/gzip/'.$lang_pack->version.'/'.$iso.'.gzip'))
							{
							...
							$errors[] = $this->l('archive cannot be downloaded from milebiz.com.');
					}
					else
						$errors[] = $this->l('archive cannot be downloaded from milebiz.com.');
				}
