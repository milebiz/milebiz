...
class YeepayCmbchina extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
	    ...
	    $this->displayName = $this->l('yeepaycmbchina');
	    $this->description = $this->l('Accepts payments with yeepaycmbchina.');
	...
	public function createSuccessedOrder($cart_id)
	{
	    ...
	    if(!$cart->OrderExists()){
			...
			$yeepaycmbchina->validateOrder($cart->id, _PS_OS_PAYMENT_, $total, $yeepaycmbchina->displayName);
		}
		...
		'<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
			<fieldset>
				...
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td colspan="2">'.$this->l('Please enter the yeepaycmbchina configuration information').'.<br /><br /></td></tr>';
		foreach($this->_fieldsList as $field => $detail){
			$this->_html .= '
			<tr>
