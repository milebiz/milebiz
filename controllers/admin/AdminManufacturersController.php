...
class AdminManufacturersControllerCore extends AdminController
{
	...
	public function initListManufacturerAddresses()
	{
		...
		$this->_select = 'cl.`name` as country, m.`name` AS manufacturer_name, ct.`name` as cityname';
		$this->_join = '
			LEFT JOIN `'._DB_PREFIX_.'country_lang` cl
				ON (cl.`id_country` = a.`id_country` AND cl.`id_lang` = '.(int)$this->context->language->id.') ';
		$this->_join .= '
			LEFT JOIN `'._DB_PREFIX_.'city` ct
				ON (ct.`id_city` = a.`id_city`) ';
		$this->_join .= '
	...
	public function renderFormAddress()
	{
		...
		$form['input'][] = array(
			...
			'size' => 33,
			'required' => true,
			'hint' => $this->l('Invalid characters:').' 0-9!<>,;?=+()@#"锟絳}_$%:'
		);
		$form['input'][] = array(
			'type' => 'text',
