...
class AdminAccessControllerCore extends AdminController
{
	...
	{
		if (_PS_MODE_DEMO_)
			throw new MileBizException(Tools::displayError('This functionality has been disabled.'));
		if ($this->tabAccess['edit'] != '1')
			throw new MileBizException(Tools::displayError('You do not have permission to edit here.'));

		if (Tools::isSubmit('submitAddAccess'))
		{
			$perm = Tools::getValue('perm');
			if (!in_array($perm, array('view', 'add', 'edit', 'delete', 'all')))
				throw new MileBizException('permission not exists');
