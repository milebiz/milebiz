...
class AdminTranslationsControllerCore extends AdminController
{
	...
	protected $link_lang_pack = 'http://www.milebiz.com/download/lang_packs/get_each_language_pack.php';
	...
	protected function writeTranslationFile($override_file = false)
	{
		...
		if (!file_exists($file_path))
			throw new MileBizException(sprintf(Tools::displayError('This file doesn\'t exists: "%s". Please create this file.'), $file_path));
		...
		if ($fd = fopen($file_path, 'w'))
		{
			...
			throw new MileBizException(sprintf(Tools::displayError('Cannot write this file: "%s"'), $file_path));
	}
	...
	public function exportTabs()
	{
		...
		if (!Tools::file_exists_cache($dir))
			if (!mkdir($dir, 0777, true))
				throw new MileBizException('The file '.$dir.' cannot be created.');
		if (!file_put_contents($path, $content))
				throw new MileBizException('File "'.$path.'" doesn\'t exists and cannot be created in '.$dir);
		if (!is_writable($path))
	...
	public function submitAddLang()
	{
		...
		if (Validate::isLangIsoCode($arr_import_lang[0]))
		{
			if ($content = Tools::file_get_contents(
				'http://www.milebiz.com/download/lang_packs/gzip/'.$arr_import_lang[1].'/'.$arr_import_lang[0].'.gzip', false,
	...
	protected function findAndWriteTranslationsIntoFile($file_name, $files, $theme_name, $module_name, $dir = false)
	{
		...
		// Default translations and MileBiz overriding themes are distinguish
		...
		if (!isset($cache_file[$theme_name.'-'.$file_name]))
		{
			...
			if (!is_writable($file_name))
				throw new MileBizException(sprintf(
			...
			if (preg_match('/^(.*).(tpl|php)$/', $file) && Tools::file_exists_cache($file_path = $dir.$file) && !in_array($file, self::$ignore_folder))
			{
				...
				foreach ($matches as $key)
				{
					if ($is_default)
					{
						...
						$pattern = '\'<{'.strtolower($module_name).'}milebiz>'.strtolower($template_name).'_'.md5($key).'\'';
					}
					else
					{
			...
			if ((preg_match('/^(.*).tpl$/', $file) || preg_match('/^(.*).php$/', $file)) && Tools::file_exists_cache($file_path = $dir.$file))
			{
				...
				foreach ($matches as $key)
				{
					$module_key = '<{'.Tools::strtolower($module_name).'}'.
						strtolower($is_default ? 'milebiz' : $theme_name).'>'.Tools::strtolower($template_name).'_'.md5($key);
	...
	public function getInformations()
	{
		...
		if (Tools::getValue('lang') || Tools::getValue('iso_code'))
		{
			...
			if (!Validate::isLangIsoCode($iso_code) || !in_array($iso_code, $this->all_iso_lang))
				throw new MileBizException(sprintf(Tools::displayError('Invalid iso code "%s"'), $iso_code));
	...
	public function postProcess()
	{
		...
		{
		 	if ($this->tabAccess['edit'] === '1')
				// Only the MileBiz team should write the translations into the _PS_TRANSLATIONS_DIR_
...
<body>
				...
				foreach ($mails as $mail_name => $content)
				{
					...
					if ($module_name_pipe_pos)
					{
						...
						if (!Validate::isModuleName($module_name))
							throw new MileBizException(sprinf(Tools::displayError('Invalid module name "%s"'), $module_name));
						$mail_name = substr($mail_name, $module_name_pipe_pos + 1);
						if (!Validate::isTplName($mail_name))
							throw new MileBizException(sprintf(Tools::displayError('Invalid mail name "%s"'), $mail_name));
					}
					...
					if (Validate::isCleanHTML($content))
					{
						...
						throw new MileBizException(Tools::displayError('HTML e-mail templates cannot contain JavaScript code.'));
				}
			}
		}
	...
	public function fileExists()
	{
		...
		if (!Tools::file_exists_cache($dir))
			if (!mkdir($dir, 0700))
				throw new MileBizException('Directory '.$dir.' cannot be created.');
		if (!Tools::file_exists_cache($dir.DIRECTORY_SEPARATOR.$file))
			if (!file_put_contents($dir.'/'.$file, "<?php\n\nglobal \$".$var.";\n\$".$var." = array();\n\n?>"))
				throw new MileBizException('File "'.$file.'" doesn\'t exists and cannot be created in '.$dir);
	...
	public function getListModules()
	{
		if (!Tools::file_exists_cache($this->translations_informations['modules']['dir']))
			throw new MileBizException(Tools::displayError('Fatal error: Module directory does not exist').'('.$this->translations_informations['modules']['dir'].')');
		if (!is_writable($this->translations_informations['modules']['dir']))
			throw new MileBizException(Tools::displayError('The module directory must be writable'));
		...
		if (!_PS_MODE_DEV_ && $this->theme_selected == self::DEFAULT_THEME_NAME)
		{
			...
			if (Tools::file_exists_cache($this->translations_informations['modules']['dir']))
				...
				$this->displayWarning(Tools::displayError('There are no modules in your copy of MileBiz. Use the Modules page to activate them or go to our Website to download additional Modules.'));
		else
	...
	public function initFormMails($no_display = false)
	{
		...
		// get all mail subjects, this method parse each files in MileBiz !!
		$subject_mail = array();
	...
	protected function writeSubjectTranslationFile($sub, $path)
	{
		if ($fd = @fopen($path, 'w'))
		{
			...
			throw new MileBizException(sprintf(Tools::displayError('Cannot write language file for e-mail subjects, path is: %s'), $path));
	}
	 ...
	 * @param boolean $is_default set it if modules are located in root/milebiz/modules folder
	 * 				  This allow to distinguish overrided milebiz theme and original module
	 */
