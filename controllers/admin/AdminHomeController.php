...
class AdminHomeControllerCore extends AdminController
{
	...
	public function ajaxProcessGetAdminHomeElement()
	{
		...
		// SCREENCAST
		if (@fsockopen('screencasts.milebiz.com', 80, $errno, $errst, AdminHomeController::TIPS_TIMEOUT))
		...
		// DISCOVER MILEBIZ
		$result['discover_milebiz'] = '<div id="block_tips">'.$this->getBlockDiscover().'</div>';

		if (@fsockopen('api.milebiz.com', 80, $errno, $errst, AdminHomeController::TIPS_TIMEOUT))
			$result['discover_milebiz'] .= '<div class="row-news"><div id="block_discover"><iframe frameborder="no" style="margin: 0px; padding: 0px; width: 100%; height:300px; overflow:hidden;" src="'.$protocol.'://api.milebiz.com/rss2/news2.php?v='._PS_VERSION_.'&lang='.$isoUser.'"></iframe></div>';
		else
			$result['discover_milebiz'] .= '';

		// SHOW TIPS OF THE DAY
		$content = @file_get_contents($protocol.'://api.milebiz.com/partner/tipsoftheday/?protocol='.$protocol.'&iso_country='.$isoCountry.'&iso_lang='.Tools::strtolower($isoUser), false, $stream_context);
		$content = explode('|', $content);
		if ($content[0] == 'OK' && Validate::isCleanHtml($content[1]))
			$result['discover_milebiz'] .= '<div id="block_partner_tips">'.$content[1].'</div></div>';
	...
	public function ajaxProcessSavePreactivationRequest()
	{
		...
		$return = @file_get_contents('http://api.milebiz.com/partner/premium/set_request.php?iso_country='.strtoupper($isoCountry).'&iso_lang='.strtolower($isoUser).'&host='.urlencode($_SERVER['HTTP_HOST']).'&ps_version='._PS_VERSION_.'&ps_creation='._PS_CREATION_DATE_.'&partner='.htmlentities(Tools::getValue('module')).'&shop='.urlencode(Configuration::get('PS_SHOP_NAME')).'&email='.urlencode($email).'&firstname='.urlencode($firstname).'&lastname='.urlencode($lastname).'&type=home');
		die($return);
	}
	...
	public function getBlockDiscover()
	{
		...
		$content = @file_get_contents($protocol.'://api.milebiz.com/partner/milebiz/milebiz-link.php?iso_country='.$isoCountry.'&iso_lang='.Tools::strtolower($isoUser).'&id_lang='.(int)Context::getContext()->language->id.'&ps_version='.urlencode(_PS_VERSION_), false, $stream_context);
