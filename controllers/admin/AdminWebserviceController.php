...
class AdminWebserviceControllerCore extends AdminController
{
		...
		if (file_exists(_PS_ROOT_DIR_.'/.htaccess'))
			$this->fields_options = array(
				'general' => array(
					...
					'fields' =>	array(
						'PS_WEBSERVICE' => array('title' => $this->l('Enable MileBiz Webservice'),
	...
	{
		if (!file_exists(_PS_ROOT_DIR_.'/.htaccess'))
			$this->warnings[] = $this->l('In order to enable the MileBiz Webservice, please generate the .htaccess file in "Preferences" > "SEO & URLs".');
		if (strpos($_SERVER['SERVER_SOFTWARE'], 'Apache') === false)
		{
			...
			if (function_exists('apache_get_modules'))
			{
				...
				if (!in_array('mod_auth_basic', $apache_modules))
					$this->warnings[] = $this->l('Please activate the Apache module \'mod_auth_basic\' to allow authentication of MileBiz webservice.');
				if (!in_array('mod_rewrite', $apache_modules))
					$this->warnings[] = $this->l('Please activate the Apache module \'mod_rewrite\' to allow the MileBiz webservice.');
			}
			else
				$this->warnings[] = $this->l('We could not check if basic authentication and rewrite extensions are activated. Please manually check if they are activated in order to use the MileBiz webservice.');
		}
		if (!extension_loaded('SimpleXML'))
			$this->warnings[] = $this->l('Please activate the PHP extension \'SimpleXML\' to allow testing of MileBiz webservice.');
