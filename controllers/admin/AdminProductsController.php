		...
		else
		{
			...
			if ($this->ajax)
				if (!isset($this->tpl_form_vars['custom_form']))
					throw new MileBizException('custom_form empty for action '.$this->tab_display);
				else
	...
	*/
	...
	public function getPackItems($product = null)
	{
		...
		if (!$product)
		{
			...
			$names = array_unique(explode('陇', $names_input));
	...
	public function initFormPack($product)
	{
		...
		else
		{
			...
			foreach ($pack_items as $pack_item)
			{
				...
				$input_namepack_items .= $pack_item['pack_quantity'].' x '.$pack_item['name'].'陇';
			}
		}
	...
	protected function _getCustomizationFieldIds($labels, $alreadyGenerated, $obj)
	{
		...
		return implode('陇', $customizableFieldIds);
	}
