...
class AdminStoresControllerCore extends AdminController
{
	public function __construct()
	{
		...
		$this->fields_list = array(
			...
			'city' => array('title' => $this->l('City'), 'width' => 100, 'filter_key' => 'ct!name'),
	...
	public function renderList()
	{
		...
		$this->_select = 'cl.`name` country, st.`name` state, ct.`name` city';
		$this->_join = '
			...
			LEFT JOIN `'._DB_PREFIX_.'city` ct ON a.id_city = ct.id_city';
	...
	protected function _getDefaultFieldsContent()
	{
		...
		$cityList = array();
		$cityList[] = array('id' => '0', 'name' => $this->l('Choose your city (if applicable)'));
	...
	public function updateOptionPsShopCityId($value)
	{
		if (!$this->errors && $value)
		{
			$city = new City($value);
			if ($city->id)
			{
				Configuration::updateValue('PS_SHOP_CITY_ID', $value);
				Configuration::updateValue('PS_SHOP_CITY', pSQL($city->name));
			}
		}
	}
}