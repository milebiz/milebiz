...
class AdminLocalizationControllerCore extends AdminController
{
	...
	public function postProcess()
	{
		if (Tools::isSubmit('submitLocalizationPack'))
		{
			...
			if (Validate::isFileName(Tools::getValue('iso_localization_pack')))
			{
				...
				$pack = @Tools::file_get_contents('http://api.milebiz.com/localization/'.$version.'/'.Tools::getValue('iso_localization_pack').'.xml');

				if (!$pack && !($pack = @Tools::file_get_contents(dirname(__FILE__).'/../../localization/'.Tools::getValue('iso_localization_pack').'.xml')))
					$this->errors[] = Tools::displayError('Cannot load localization pack (from milebiz.com and from your local folder "localization")');
	...
	public function renderForm()
	{
		...
		$xml_localization = Tools::simplexml_load_file('http://api.milebiz.com/rss/localization.xml');
		if (!$xml_localization)
		{
		...
		if (!$localizations_pack)
			return $this->displayWarning($this->l('Cannot connect to milebiz.com'));
