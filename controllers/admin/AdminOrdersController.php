...
class AdminOrdersControllerCore extends AdminController
{
	...
	public function __construct()
	{
		...
		if (Tools::isSubmit('id_order'))
		{
			...
			if (!Validate::isLoadedObject($order))
				throw new MileBizException('Cannot load Order object');
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('id_order') && Tools::getValue('id_order') > 0)
		{
			...
			if (!Validate::isLoadedObject($order))
				throw new MileBizException('Can\'t load Order object');
		}
				...
				else
				{
					...
					if ($order_carrier->update())
					{
						...
						if (!Validate::isLoadedObject($customer))
							throw new MileBizException('Can\'t load Customer object');
						if (!Validate::isLoadedObject($carrier))
							throw new MileBizException('Can\'t load Carrier object');
						$templateVars = array(
			...
			if ($this->tabAccess['edit'] === '1')
			{
				if (Tools::getValue('new_currency') != $order->id_currency && !$order->valid)
				{
					...
					if (!Validate::isLoadedObject($currency))
						throw new MileBizException('Can\'t load Currency object');
				...
				if (Validate::isLoadedObject($order_cart_rule) && $order_cart_rule->id_order == $order->id)
				{
					if ($order_cart_rule->id_order_invoice)
					{
						...
						if (!Validate::isLoadedObject($order_invoice))
							throw new MileBizException('Can\'t load Order Invoice object');
	...
	public function renderView()
	{
		$order = new Order(Tools::getValue('id_order'));
		if (!Validate::isLoadedObject($order))
			throw new MileBizException('object can\'t be loaded');
