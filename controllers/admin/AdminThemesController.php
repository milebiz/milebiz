...
class AdminThemesControllerCore extends AdminController
{
	 ...
	 * and also checks milebiz current configuration if not match.
	 * @static
	 */
		...
		'ccc' => array(
			'attributes' => array(
				'available' => array(
					'value' => 'true',
					/*
					 * accepted attribute value if value doesnt match, milebiz configuration value must have thoses values
					*/
	...
	public function ajaxProcessGetAddonsThemes()
	{
		...
		if (@fsockopen('addons.milebiz.com', 80, $errno, $errst, 3))
			@readfile('http://addons.milebiz.com/adminthemes.php?lang='.$this->context->language->iso_code);
	}
	...
	protected function _isThemeCompatible($theme_dir)
	{
		...
		{
			$this->errors[] .= Tools::displayError('config.xml theme file has not been created for this version of milebiz.');
			$return = false;
		}
		return $return;
	}
